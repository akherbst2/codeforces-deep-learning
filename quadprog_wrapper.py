...
def solve_quadprog(hessian, weights, eq_coeffs, eq_constants, ineq_coeffs, ineq_constants, lower_bounds, upper_bounds):
    ...
    try:
        ...
        print ("Warning: Quadratic program solver exited with a numerical error (quadprog can be sensitive to very non-separable problems). Guessing all-zeros solution.")
        sol = np.zeros(n)
